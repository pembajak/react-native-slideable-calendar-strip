...
export default ({ isChinese, weekStartsOn , highlightColor ,normalColor , dayStyle,selectedDate}) => {
  ...
  return (
    <View style={{
      ...
      {weekTranformed.map((day,index) => {
        var color = (selectedDate.getDay()-1) == index ? highlightColor : normalColor
        return (
          <View style={{
            flex: 1,
            height: '100%',
